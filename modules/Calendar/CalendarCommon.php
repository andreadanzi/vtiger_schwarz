...
function getActivityMailInfo($return_id,$status,$activity_type)
{
	...
	$rel_qry = "select case vtiger_crmentity.setype when 'Leads' then vtiger_leaddetails.lastname when 'Accounts' then vtiger_account.accountname when 'Potentials' then vtiger_potential.potentialname when 'Quotes' then vtiger_quotes.subject when 'PurchaseOrder' then vtiger_purchaseorder.subject when 'SalesOrder' then vtiger_salesorder.subject when 'Invoice' then vtiger_invoice.subject when 'Campaigns' then vtiger_campaign.campaignname when 'HelpDesk' then vtiger_troubletickets.title when 'Project' then vtiger_project.projectname when 'ProjectTask' then vtiger_projecttask.projecttaskname end as relname 
...
left join vtiger_projecttask on vtiger_projecttask.projecttaskid = vtiger_seactivityrel.crmid 
