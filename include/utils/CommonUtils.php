...
function getQuickCreateModules() {
	...
	$qc_query = "select distinct vtiger_tab.tablabel,vtiger_tab.name from vtiger_field inner join vtiger_tab on vtiger_tab.tabid = vtiger_field.tabid where quickcreate in (0,2) and vtiger_tab.presence != 1 order by vtiger_tab.tablabel";
	// END
...
// danzi.tn@20140324 costruzione url per visualizzare i progetti e le pratiche collegate ad un Cliente
function getRelatedSearchEntitiesUrl($module, $record_id, $related_module) {
	global $adb;
	$related_field = "";
	$related_value = "";
	if($module == 'Accounts' ) {
		...
		if ($result && $adb->num_rows($result)>0) {
			$related_value = $adb->query_result($result,0,'accountname');
		}
	}
	
	if ( $related_module == 'Project') {
		$related_field = "linktoaccountscontacts";
	} elseif ($related_module == 'ProjectTask') {
		$related_field = "account_id";
	}
	$url = getRelatedUrl($related_module,$related_field,$related_value);	
	return $url;
}

function getRelatedUrl($related_module,$related_field,$related_value) {
	$url="index.php?module=";
	$url.=urlencode($related_module)."&action=ListView&query=true&search=true";
	$url.="&search_field=".urlencode($related_field)."&searchtype=BasicSearch&search_text=".urlencode($related_value);
	return $url;
}
// danzi.tn@20140324e
