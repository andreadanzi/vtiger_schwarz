	...
	function getListViewEntries($focus, $module,$result,$navigationInfo,$skipActions=false) {
		...
		for ($i = 0; $i < $rowCount; ++$i) {
			...
			foreach ($listViewFields as $fieldName) {
				...
				} elseif ($field->getUIType() == '1017') { // danzi.tn@20140324 gestione dei link a entità correlate
					$url_val = getRelatedSearchEntitiesUrl($module, $recordId, $rawValue);
					$value = '<a href="' . $url_val . '">' . getTranslatedString($rawValue, $rawValue) . '</a>';
